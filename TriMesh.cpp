...
glm::mat4 TriMesh::getModelMatrix() {
	...
	model = glm::scale(model, getScale());	// 扩大缩小矩阵
	return model;
}
...
void TriMesh::storeFacesPoints() {
	...
	if(do_normalize_size) {
		// 记录物体包围盒大小，可以用于大小的归一化 将对应的物体的大小归一化
		// 先获得包围盒的对角顶点
		float max_x = -FLT_MAX;
		float max_y = -FLT_MAX;
		float max_z = -FLT_MAX;
		...
		center = glm::vec3((min_x + max_x) / 2.0, (min_y + max_y) / 2.0, (min_z + max_z) / 2.0); // 求出中心点的坐标值

		diagonal_length = length(up_corner - down_corner); // 求出物体中距离最长的边

		float min = INT_MAX,max = INT_MIN;
		for(int i = 0; i < vertex_positions.size(); i++) {
			vertex_positions[i] = (vertex_positions[i] - center) / diagonal_length;  // 对所有点进行一个归一化处理 更新所有点的距离
		}
	}
	// 计算法向量
...
// Light
glm::mat4 Light::getShadowProjectionMatrixZ() {
	// 实现了Z=0平面上的阴影投影矩阵
	float lx, ly, lz;
