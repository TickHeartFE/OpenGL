...
void MeshPainter::addMesh(TriMesh* mesh, const std::string& name, const std::string& texture_image, const std::string& vshader, const std::string& fshader) {
	...
	openGLObject object;
	// 绑定openGL对象，并传递顶点属性的数据 之后绘制每一个对象的值
...
// 对其计算阴影
void MeshPainter::drawMeshS(TriMesh* mesh, openGLObject& object, Light* light, Camera* camera) {

	// 相机矩阵计算 位于每次鼠标动就更新欧拉角 所以每次display都会更新摄像机的位置
	camera->updateCamera();
	...
	// 将着色器 isShadow 设置为0，表示正常绘制的颜色，如果是1着表示阴影 传递数据到着色器部分
	...
	// 绘制阴影的代码 传入着色器进行渲染 对其进行变换然后绘制 绘制阴影的代码
...
void MeshPainter::drawMeshes(Light* light, Camera* camera) {
	for(int i = 0; i < meshes.size(); i++) {
		// 判断是否显示物体的阴影 如果是shadow的话就进行影子渲染
		if(mesh_names[i] == "hasShadow") {
			drawMeshS(meshes[i], opengl_objects[i], light, camera);
		} else {
